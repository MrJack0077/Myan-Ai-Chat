...
export default function LoginPage() {
  ...
  const handleSetupAdmin = async () => {
    ...
    try {
      ...
      // Let handleFirestoreError print and throw if it's a Firestore-like error, otherwise use standard message
      if (err.code || err.message?.includes('offline') || err.message?.includes('permission')) {
        try {
          handleFirestoreError(err, OperationType.WRITE, 'users');
        } catch (fbErr: any) {
           setError(fbErr.message || 'Failed to setup super admin due to database error');
        }
      } else {
