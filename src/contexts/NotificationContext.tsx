...
export const NotificationProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    if (user?.shopId) {
      const unsubscribe = orderService.subscribeToOrders(user.shopId, (orders) => {
  ...
  useEffect(() => {
    if (user?.shopId) {
      const unsubscribe = customerService.subscribeToBotUsers(user.shopId, (botUsers) => {
        const shopBotUsers = botUsers;
  ...
  useEffect(() => {
    if (user?.shopId) {
      const unsubscribe = inventoryService.subscribeToItems(user.shopId, (items) => {
        if (isInitialLoad.current) {
          items.forEach(item => {
