...
import { Order, OrderStatus, VendorItem, OrderItem, PaymentStatus } from '../../types';
import { 
  getOrders, 
  updateOrderStatus, 
  createOrder, 
...
import { BotUser } from '../../services/firebaseService';
...
export default function OrderManager({ shopId, currency = 'USD' }: { shopId: string, currency?: string }) {
  ...
  useEffect(() => {
    if (shopId) {
      const unsubscribe = subscribeToOrders(shopId, (data) => {
        setOrders(data);
        setIsLoading(false);
      });
  ...
  const fetchBotUsers = async () => {
    try {
      const data = await getBotUsers(shopId);
      setBotUsers(data);
    } catch (error) {
  ...
  const handleUpdatePaymentStatus = async (orderId: string, status: PaymentStatus) => {
    setIsUpdating(true);
    try {
      await updateOrderPaymentStatus(shopId, orderId, status);
  ...
  const fetchCustomerHistory = async (phone: string) => {
    ...
    try {
      const history = await getCustomerOrderHistory(shopId, phone);
  ...
  const handleCreateOrder = async (e: React.FormEvent) => {
    ...
    setIsUpdating(true);
    try {
      await createOrder(shopId, newOrder as any);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
        ...
        <div className="flex gap-2 w-full md:w-auto">
          <button 
            onClick={() => { fetchOrders(); fetchBotUsers(); }}
                      ...
                      <td className="px-6 py-4 text-right">
                        {user.order_state === 'CONFIRMING' && (
                          <button 
                            onClick={async () => {
                              try {
                                await finalizeBotOrder(shopId, user.id);
