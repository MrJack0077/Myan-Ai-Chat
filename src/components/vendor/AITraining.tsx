...
import { updateShopAIConfig } from '../../services/firebaseService';
...
export default function AITraining({ initialConfig, shopId }: { initialConfig?: ShopAIConfig, shopId?: string }) {
  ...
  const handleSyncInventory = async () => {
    ...
    setIsSyncing(false);
    showToast('Inventory data synced with AI Agent!', 'success');
  };
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white p-8 rounded-3xl border border-zinc-200 shadow-sm space-y-6">
            <div className="flex items-center justify-between pb-6 border-b border-zinc-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-xl font-bold text-zinc-900">{t('ai_training.lead_capture')}</h3>
                  <p className="text-sm text-zinc-500">{t('ai_training.lead_capture_desc')}</p>
                </div>
              </div>
              <button
                onClick={() => handleSave(t('ai_training.lead_capture'))}
                disabled={isSaving}
                className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-xl text-sm font-bold transition-all disabled:opacity-50"
              >
                <Save className="w-4 h-4" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-4">
                <h4 className="text-sm font-bold text-zinc-900">{t('ai_training.data_collection')}</h4>
                <div className="flex items-center justify-between p-4 bg-zinc-50 rounded-2xl border border-zinc-100">
                  <div className="flex items-center gap-3">
                    <Mail className="w-4 h-4 text-zinc-400" />
                    <span className="text-sm font-medium">{t('ai_training.capture_email')}</span>
                  </div>
                  <button 
                    onClick={() => setConfig({ ...config, handoffRules: { ...config.handoffRules!, captureEmail: !config.handoffRules?.captureEmail } })}
                    className={`w-10 h-5 rounded-full transition-all relative ${config.handoffRules?.captureEmail ? 'bg-indigo-600' : 'bg-zinc-300'}`}
                  >
                    <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${config.handoffRules?.captureEmail ? 'left-6' : 'left-1'}`} />
                  </button>
                </div>
                <div className="flex items-center justify-between p-4 bg-zinc-50 rounded-2xl border border-zinc-100">
                  <div className="flex items-center gap-3">
                    <Phone className="w-4 h-4 text-zinc-400" />
                    <span className="text-sm font-medium">{t('ai_training.capture_phone')}</span>
                  </div>
                  <button 
                    onClick={() => setConfig({ ...config, handoffRules: { ...config.handoffRules!, capturePhone: !config.handoffRules?.capturePhone } })}
                    className={`w-10 h-5 rounded-full transition-all relative ${config.handoffRules?.capturePhone ? 'bg-indigo-600' : 'bg-zinc-300'}`}
                  >
                    <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${config.handoffRules?.capturePhone ? 'left-6' : 'left-1'}`} />
                  </button>
                </div>
              </div>

              <div className="space-y-4">
                <h4 className="text-sm font-bold text-zinc-900">{t('ai_training.handoff_thresholds')}</h4>
                <div>
                  <label className="block text-[10px] font-bold text-zinc-400 uppercase tracking-wider mb-1.5">{t('ai_training.min_order_value')}</label>
                  <input
                    type="number"
                    value={config.handoffRules?.minPriceThreshold}
                    onChange={(e) => setConfig({ ...config, handoffRules: { ...config.handoffRules!, minPriceThreshold: parseInt(e.target.value) } })}
                    ...
                    placeholder="e.g. 100"
                  />
                  <p className="text-[10px] text-zinc-400 mt-1">{t('ai_training.min_order_desc')}</p>
                </div>
              </div>
            </div>

            <div className="space-y-4 pt-4 border-t border-zinc-100">
              <h4 className="text-sm font-bold text-zinc-900">{t('ai_training.trigger_keywords')}</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-[10px] font-bold text-zinc-400 uppercase tracking-wider mb-1.5">{t('ai_training.handoff_triggers')}</label>
                  <input
                    type="text"
                    value={config.handoffRules?.triggerKeywords.join(', ')}
                    onChange={(e) => setConfig({ ...config, handoffRules: { ...config.handoffRules!, triggerKeywords: e.target.value.split(',').map(k => k.trim()) } })}
                    className="w-full px-4 py-2 rounded-xl border border-zinc-200 focus:ring-2 focus:ring-indigo-500 outline-none transition-all text-sm"
                    placeholder="human, agent, manager..."
                  />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-zinc-400 uppercase tracking-wider mb-1.5">{t('ai_training.urgency_triggers')}</label>
                  <input
                    type="text"
                    value={config.handoffRules?.urgencyKeywords.join(', ')}
                    onChange={(e) => setConfig({ ...config, handoffRules: { ...config.handoffRules!, urgencyKeywords: e.target.value.split(',').map(k => k.trim()) } })}
                    className="w-full px-4 py-2 rounded-xl border border-zinc-200 focus:ring-2 focus:ring-indigo-500 outline-none transition-all text-sm"
                    placeholder="urgent, emergency, broken..."
                  />
                </div>
              </div>
            </div>
          ...
          <div className="bg-white p-8 rounded-3xl border border-zinc-200 shadow-sm space-y-6">
            <div className="flex items-center gap-3 pb-6 border-b border-zinc-100">
              ...
              <div>
                <h3 className="text-xl font-bold text-zinc-900">{t('ai_training.learning_center')}</h3>
                <p className="text-sm text-zinc-500">{t('ai_training.learning_center_desc')}</p>
              </div>
            </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-4">
                  <h4 className="text-sm font-bold text-zinc-900 flex items-center gap-2">
                    ...
                    {t('ai_training.unanswered_questions')}
                  </h4>
                  <span className="text-[10px] font-bold bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full">
                    {config.learningCenter?.unansweredQuestions.length} {t('ai_training.pending')}
                  </span>
                </div>
                <div className="space-y-3">
                  {config.learningCenter?.unansweredQuestions.map((q) => (
                    <div key={q.id} className="p-4 bg-zinc-50 rounded-2xl border border-zinc-100 flex items-center justify-between group">
                      <p className="text-sm text-zinc-700 font-medium">{q.question}</p>
                      <button className="flex items-center gap-1 text-xs font-bold text-indigo-600 hover:bg-indigo-50 px-3 py-1.5 rounded-lg transition-all">
                        {t('ai_training.provide_answer')}
                        <ArrowRight className="w-3 h-3" />
                      </button>
                    </div>
                  ))}
                  {(!config.learningCenter?.unansweredQuestions || config.learningCenter.unansweredQuestions.length === 0) && (
                    <div className="text-center py-8 bg-zinc-50 rounded-2xl border border-dashed border-zinc-200">
                      <p className="text-sm text-zinc-400 italic">{t('ai_training.all_answered')}</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="pt-6 border-t border-zinc-100">
                <div className="flex items-center justify-between mb-4">
                  <h4 className="text-sm font-bold text-zinc-900 flex items-center gap-2">
                    ...
                    {t('ai_training.recent_corrections')}
                  </h4>
                </div>
                <div className="text-center py-8 bg-zinc-50 rounded-2xl border border-dashed border-zinc-200">
                  <p className="text-sm text-zinc-400 italic">{t('ai_training.no_corrections')}</p>
                </div>
              </div>
            </div>
          </div>
