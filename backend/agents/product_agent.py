...
async def run_product_agent(user_msg, tool_info, ai_cfg, policies, profile, base_model_name, chat_history, past_purchases=None, delivery_info=None, payment_info=None, media_parts=None, shop_doc_id=None):
    ...
    extra_ctx = [
        ...
        "⚠️ MATCHING: Match products by name/brand even with minor typos (eg. 'buld' → 'Bulb', 'samsang' → 'Samsung').\n"
        "   Be generous with matching — if it sounds similar, it's probably the right product.\n"
        "⚠️ PHOTO RULE: If customer sends a photo/image — first try to identify what's in it.\n"
        "   If the photo seems like a product NOT in your shop → say politely it's not available.\n"
        "   If the photo matches a shop product → describe it and offer to order.\n"
        "⚠️ VAGUE QUERIES: 'this one', 'it available?', 'ရှိလား' with no product name →\n"
